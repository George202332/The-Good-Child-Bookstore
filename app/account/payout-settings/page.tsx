...
export default async function PayoutSettingsPage() {
  ...
  const [payouts, monthlyRows, statCards] = await Promise.all([
    getMyPayoutRequests(),
  ...
  return (
    <DashboardShell role={role} activeKey="payout-settings" displayName={session.user.name ?? ""}>
      ...
      <div className="stat-grid" style={{ marginBottom: 28 }}>
        ...
        <div className="stat-card stat-card-due">
          ...
          <div className="stat-sub">
            {statCards.pendingStatus === "Paid" ? "Already paid on the 15th" : "Last month's earnings, due the 15th"}
          </div>
        </div>
      </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr>
                ...
                <th style={TABLE_HEAD_STYLE}>Referral Revenue<ColHelp text="5% of company revenue from authors you personally referred onto the platform, earned this month." /></th>
                <th style={TABLE_HEAD_STYLE}>Promotion Revenue<ColHelp text="10% commission earned this month from copies sold through your own affiliate promotional links." /></th>
                <th style={TABLE_HEAD_STYLE}>Payout Date<ColHelp text="This month's earnings become payable on the 15th of the following month." /></th>
            ...
            <tbody>
              {monthlyRows.map((r) => (
                <tr key={r.monthKey}>
                  ...
                  <td style={TABLE_CELL_STYLE}><span className="age-pill">{r.status === "Paid" ? "Paid" : "Pending payout"}</span></td>
      ...
      <h3 style={{ fontSize: 16, margin: "28px 0 14px" }}>Payout requests</h3>
      {payouts.length === 0 ? (
